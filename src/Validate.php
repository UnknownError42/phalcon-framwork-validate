...
class Validate extends Validation
{
    ...
    protected static $types = [
        ...
        'alnum' => [
            'validator' => \Phalcon\Validation\Validator\Alnum::class,
            'message' => ':field 必须只包含字母数字字符'
        ],
        // Check for alphabetic character(s)
        'alpha' => [
            'validator' => \Phalcon\Validation\Validator\Alpha::class
        ],
        // Validates that a value is between an inclusive range of two values.
        'between' => [
            'validator' => \Phalcon\Validation\Validator\Between::class
        ],
        // Calls user function for validation
        'callback' => [
            'validator' => \Phalcon\Validation\Validator\Callback::class
        ],
        // Checks that two values have the same value
        'confirmation' => [
            'validator' => \Phalcon\Validation\Validator\Confirmation::class
        ],
        // Checks if a value has a valid credit card number
        'creditcard' => [
            'validator' => \Phalcon\Validation\Validator\CreditCard::class
        ],
        // Checks if a value is a valid date
        'date' => [
            'validator' => \Phalcon\Validation\Validator\Date::class
        ],
        // Check for numeric character(s)
        'digit' => [
            'validator' => \Phalcon\Validation\Validator\Digit::class
        ],
        // Checks if a value has a correct e-mail format
        'email' => [
            'validator' => \Phalcon\Validation\Validator\Email::class
        ],
        // Check if a value is not included into a list of values
        'exclusionin' => [
            'validator' => \Phalcon\Validation\Validator\ExclusionIn::class
        ],
        // Checks if a value has a correct file
        'file' => [
            'validator' => \Phalcon\Validation\Validator\File::class
        ],
        // Checks if a value is identical to other
        'identical' => [
            'validator' => \Phalcon\Validation\Validator\Identical::class
        ],
        // Check if a value is included into a list of values
        'inclusionin' => [
            'validator' => \Phalcon\Validation\Validator\InclusionIn::class
        ],
        // Check for a valid numeric value
        'numericality' => [
            'validator' => \Phalcon\Validation\Validator\Numericality::class
        ],
        // Validates that a value is not null or empty string
        'presenceof' => [
            'validator' => \Phalcon\Validation\Validator\PresenceOf::class
        ],
        // Allows validate if the value of a field matches a regular expression
        'regex' => [
            'validator' => \Phalcon\Validation\Validator\Regex::class
        ],
        // Validates that a string has the specified maximum and minimum constraints
        'stringlength' => [
            'validator' => \Phalcon\Validation\Validator\StringLength::class
        ],
        // Check that a field is unique in the related table
        'uniqueness' => [
            'validator' => \Phalcon\Validation\Validator\Uniqueness::class
        ],
     ...
     * 全部：alnum（字母数字）、alpha（字母）、between、callback、confirmation、creditcard、date、digit（数字）、email、exclusionin、file、identical（检查一个值与指定固定值是否相同）、idnumber(身份证号)、inclusionin、numericality（数值）、presenceof、regex、stringlength、uniqueness、url
     *
     * 错误提示：
     ...
     *
     * 详细说明：
     * 当验证类型为：alnum、alpha、creditcard、digit、email、idnumber、numericality、url
     ...
     *            验证失败后是否停止后续验证
     * @return \PhalconValidate\Validate
     */
    public function addRules(array $rules, bool $cancelOnFail = true)
    {
        $types = self::$types;
        foreach ($rules as $rule) {
            ...
            $options['cancelOnFail'] = $cancelOnFail;
            switch ($type) {
                case 'alnum':
                case 'alpha':
                case 'creditcard':
            ...
            if (! isset($options['message']) && isset($types[$type]['message'])) {
                $options['message'] = $types[$type]['message'];
            }
